...
func NewTypingRepository() *TypingRepository {
	return &TypingRepository{
		typing: make(map[uuid.UUID]map[uuid.UUID]time.Time),
	}
}

func (tr *TypingRepository) SetTyping(chatID, userID uuid.UUID) {
	tr.mu.Lock()
	defer tr.mu.Unlock()
...
func (tr *TypingRepository) GetTypingUsers(chatID uuid.UUID, timeout time.Duration) []uuid.UUID {
	tr.mu.Lock()
	defer tr.mu.Unlock()
...
func (tr *TypingRepository) Cleanup(timeout time.Duration) {
	tr.mu.Lock()
	defer tr.mu.Unlock()
