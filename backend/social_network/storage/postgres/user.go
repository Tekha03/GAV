...
func (r *UserRepository) WithinTransaction(ctx context.Context, fn func(context.Context) error) error {
...
func (r *UserRepository) FindWalkingNearby(ctx context.Context, centerLat, centerLon float64, radiusMeters float64, activeAfter time.Time) ([]*dog.Dog, error) {
	...
	FROM dogs
	JOIN walk_sessions ON CAST(walk_sessions.user_id AS TEXT) = dogs.owner_id
