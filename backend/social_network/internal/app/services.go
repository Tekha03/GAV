...
func initServices(repos *Repositories, jwtConfig auth.JWTConfig, storage media.Storage, notificationHub *notification.Hub) (*Services, error) {
	...
	var err error
	s.User, err = user.NewService(repos.User)
	if err != nil {
		return nil, err
	}
	...
	s.Auth, err = auth.NewService(s.User, s.Token, jwtConfig, &auth.PasswordHasher{})
	if err != nil {
		return nil, err
	}
	s.Profile, err = profile.NewService(repos.Profile)
	if err != nil {
		return nil, err
	}
	s.Stats, err = stats.NewService(repos.Stats)
	if err != nil {
		return nil, err
	}
	s.Post, err = post.NewService(repos.Post, s.Stats)
	if err != nil {
		return nil, err
	}
	s.Feed, err = feed.NewService(repos.Post)
	if err != nil {
		return nil, err
	}
	s.Media, err = media.NewService(storage)
	if err != nil {
		return nil, err
	}
	s.Comment, err = comment.NewService(repos.Comment, s.Stats)
	if err != nil {
		return nil, err
	}
	s.Like, err = like.NewService(repos.Like, s.Stats)
	if err != nil {
		return nil, err
	}
	s.Follow, err = follow.NewService(repos.Follow, s.Stats)
	if err != nil {
		return nil, err
	}
	...
	s.Vaccination, err = vaccination.NewService(repos.Vaccination)
	if err != nil {
		return nil, err
	}
	s.Settings, err = settings.NewService(repos.Settings)
	if err != nil {
		return nil, err
	}
